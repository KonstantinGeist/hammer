...
static void test_can_create_string_from_string_builder_with_start_index_and_length_in_bytes()
{
    hmAllocator allocator;
    HM_TEST_INIT_ALLOC(&allocator);
    HM_TEST_TRACK_OOM(&allocator, HM_FALSE);
    ...
    HM_TEST_ASSERT_OK(err);
    HM_TEST_TRACK_OOM(&allocator, HM_TRUE);
    err = hmStringBuilderAppendCString(&string_builder, "Hello, World!");
    HM_TEST_ASSERT_OK_OR_OOM(err);
    hmString string;
    err = hmStringBuilderToStringWithStartIndexAndLengthInBytes(&string_builder, HM_NULL, 1, 4, &string);
    HM_TEST_ASSERT_OK_OR_OOM(err);
    HM_TEST_ASSERT(hmStringEqualsToCString(&string, "ello"));
    err = hmStringDispose(&string);
    HM_TEST_ASSERT_OK_OR_OOM(err);
    err = hmStringBuilderToStringWithStartIndexAndLengthInBytes(&string_builder, HM_NULL, 5, 100, &string);
    HM_TEST_ASSERT(err == HM_ERROR_OUT_OF_RANGE);
    err = hmStringBuilderToStringWithStartIndexAndLengthInBytes(&string_builder, HM_NULL, 100, 5, &string);
    HM_TEST_ASSERT(err == HM_ERROR_OUT_OF_RANGE);
HM_TEST_ON_FINALIZE
    err = hmStringBuilderDispose(&string_builder);
    HM_TEST_ASSERT_OK(err);
...
HM_TEST_SUITE_BEGIN(string_builders)
    ...
    HM_TEST_RUN(test_can_create_string_from_string_builder_with_start_index_and_length_in_bytes)
